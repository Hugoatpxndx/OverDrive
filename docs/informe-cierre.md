...
enviar su canción a una playlist y el curador gana 1 token al aceptarla. El
sistema incluye autenticación **JWT con roles**, integración **OAuth 2.0 con
Spotify**, pruebas automatizadas (**57 tests, cobertura ≥ 80 %**), un pipeline
**CI/CD** y análisis de **seguridad (OWASP ZAP)** y **calidad (SonarQube)**.
...
| 4 | Propuestas con tokens | Envío, aceptación, descuento de tokens y regla anti-fraude | ✅ | Sin desviación |
| 5 | Pruebas unitarias ≥ 80 % | **57 tests**, 80.6 % statements / 81.4 % líneas | ✅ | Sin desviación |
| 6 | Pipeline CI/CD | 5 jobs: tests, build, seguridad, Sonar, **despliegue a entorno de prueba** | ✅ | +2 días |
| 7 | Integración Spotify (OAuth) | Conexión, import de playlists y seguidores reales | ✅ | **+1.5 días** |
| 8 | Verificación de email / login social | Descartado por alcance | ⚠️ | No ejecutado (decisión) |
| 9 | Validación de track contra API Spotify | Diferido a Sprint 3 | ⚠️ | No ejecutado (decisión) |
...
- **El alcance debe acotarse**: se priorizó calidad/seguridad/despliegue sobre
  funcionalidades extra (verificación de email, login social) para cumplir el
  objetivo SMART.
...
|---|---|---|---|
| Validar tracks y propiedad real vía **Spotify Web API** | 100 % de envíos validados; 0 URLs inválidas | Sprint 3 (2 sem) | Dev backend |
...
6. **Transparencia y trazabilidad** con auditoría de acciones (quién propuso/aceptó
   qué y cuándo) para reforzar la confianza de la comunidad.
